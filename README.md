...
| 写作 | 排版 | 发布 |
| --- | --- | --- |
| 直接管理本地 Markdown 文件夹 | 多套内置主题与实时预览 | 一键复制到微信公众号 |
| CodeMirror 编辑器与语法提醒 | 编辑、分屏、预览三种视图 | 知乎复制方案仍在调试 |
...
公众号与 Markdown 共用一个清晰的发布入口。TypX 会针对公众号编辑器处理公式、图片和样式，让预览中的效果尽可能稳定地带到发布平台。知乎复制入口目前仍在调试。
