...
- **Markdown 语法提醒**：可疑的 `**` 没有形成粗体时显示轻提示，只提醒、不自动修改，也可在设置中关闭
- **三栏界面**：文件树右键可复制文件绝对路径/用系统默认程序打开/在资源管理器定位/标记「已发布公众号」（绿色徽章 + 统计，防止重复上传）；文件树 · CodeMirror 6 编辑器（Ctrl+S 保存，右键剪切/复制/粘贴/全选）· 同步滚动预览
- **视图模式**：编辑 / 分屏 / 预览三档切换；点击预览内容会精确选中并高亮对应 Markdown 源码，点击本地图片直接在资源管理器中定位原文件
